      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (existingVote) {
          ...
          if (existingVote.voteType === input.voteType) {
            ...
            // If different vote type, update the vote (switch vote)
            ...
            return { action: 'updated' as const, voteType: input.voteType };
          }
        } else {
          // Create new vote
          ...
          return { action: 'created' as const, voteType: input.voteType };
        }
      }),
    getByComment: publicProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const userVote = ctx.userId
          ? votes.find((v) => v.userId === ctx.userId)?.voteType || 'NONE'
          : 'NONE';

...
export function CommentItem({ comment }: CommentItemProps) {
  const { votesData, handleUpvote, handleDownvote } = useCommentVotes(comment.id);
  ...
  return (
    <div className="flex flex-col gap-2 py-2">
      ...
      <ReactionStatistics
        userVote={votesData?.userVote as 'none' | 'up' | 'down'}
        upvotes={votesData?.upvotes}
        upvoters={votesData?.upvoters}
        downvotes={votesData?.downvotes}
        downvoters={votesData?.downvoters}

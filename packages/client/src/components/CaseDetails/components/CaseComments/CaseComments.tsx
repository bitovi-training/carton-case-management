...
export function CaseComments({ caseData }: CaseCommentsProps) {
      ...
      if (previousCase && currentUser) {
        const updatedComments = previousCase.comments?.map((comment) => {
          if (comment.id === variables.commentId) {
            // Remove any existing vote by current user
  ...
  const unvoteMutation = trpc.comment.unvote.useMutation({
    onMutate: async (variables) => {
      // Cancel outgoing refetches
      await utils.case.getById.cancel({ id: caseData.id });
  ...
  const handleVote = (commentId: string, voteType: 'UP' | 'DOWN') => {
    ...
    if (userVote?.voteType === voteType) {
      ...
      // Otherwise, vote (this will replace any existing vote)
        ...
        {caseData.comments && caseData.comments.length > 0 ? (
          caseData.comments.map((comment) => {
            ...
            const userVote = currentUser 
              ...
              : null;
            const userVoteType = userVote?.voteType === 'UP' 
              ? 'up' 
